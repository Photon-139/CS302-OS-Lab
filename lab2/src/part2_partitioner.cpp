...
int main(int argc, char **argv)
{
    ...
    if(curr_region<=max_chunk_size){
        auto searcher_pid = fork();
        if(searcher_pid==0){
            string start_str = to_string(search_start_position);
            string end_str = to_string(search_end_position);
            execl("./part2_searcher.out", "part2_searcher.out", file_to_search_in, pattern_to_search_for, start_str.c_str(), end_str.c_str(), NULL);
    ...
    if(left_pid==0){
        ...
        execl("./part2_partitioner.out", "part2_partitioner.out", file_to_search_in, pattern_to_search_for, left_start_str.c_str(), left_end_str.c_str(), argv[5], NULL);
        cout << "Error while calling execl\n";
        exit(-1);
    }
    ...
    if(right_pid==0){
        ...
        execl("./part2_partitioner.out", "part2_partitioner.out", file_to_search_in, pattern_to_search_for, right_start_str.c_str(), right_end_str.c_str(), argv[5], NULL);
        cout << "Error while calling execl\n";
        exit(-1);
    }
